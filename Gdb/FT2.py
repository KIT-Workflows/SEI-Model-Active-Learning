...
def KernPCA(data):
    ...
    plt.savefig(f'{modelout}/TDATA_kpc_res', dpi=300, bbox_inches='tight')
...
def InvInf(datapd, trsize= 0.7, TI=1000, EV=100, XX= [3, 5, 7, 10, 15, 20] ):
    ...
    for G in XXX:
        ...
        a0 = train_y[train_y ==0].sample(G)
        a1 = train_y[train_y ==1].sample(G)
        a2 = train_y[train_y ==2].sample(min(len(train_y[train_y ==2]),G))
        ...
        for i in range(training_iterations +1):
            ...
            if i%EV==0:
                ...
                with gpytorch.settings.fast_pred_var(), torch.no_grad():
                    ...
                    noise_var = likelihood.noise.cpu().detach().numpy()[0]
                ...
                print(matrix)
                np.save(arr=matrix, file=f'{modelout}/model-{round(G/ld*100)}ConfMatrix')
                matrixc = matrix.copy()
        ...
        torch.save(model.state_dict(), f'{modelout}/GPmodel-{round(G/ld*100)}prc.pth')
        np.save(arr=inducing_points.cpu(), file=f'{modelout}/model-{round(G/ld*100)}prc-indc_point')
        np.save(arr=noise_var, file=f'{modelout}/model-{round(G/ld*100)}prc-noise_var')
        return errcollect, model, likelihood, matrixc
        

def Mload(Tdata, VV = 5):
...
def Evaalcuda(model, likelihood, inputP):

    model.eval()
    likelihood.eval()
    inputP = torch.Tensor(inputP).cuda()
    ...
    ypred = torch.argmax(pred_means , dim=0).cpu().numpy()
    return ypred, probabilities.cpu().numpy()
