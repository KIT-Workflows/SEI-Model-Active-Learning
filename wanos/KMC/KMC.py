...
def Analy(datadir, OII, File, br, brINDEX, Freq=0, Concentration=0, Porosity=0, Thickness=0, Flux=1):
    ...
    if Concentration:
        ...
        # Load JSON file
        """with open(datadir + f'/ConcA{brINDEX}.json', 'r') as f:
            data_json = json.load(f)

        # output dict for kmc properties

        output_kmc = {}
        output_kmc["data_json"] = data_json
        """
        # with open('output_kmc.yml', 'w') as f:
        #     yaml.dump(data_json, f)
    ...
    if option == 2:
        ...
        temp_dict["x"] = [i[0] for i in l[q][0]]
        temp_dict["y"] = [i[1] for i in l[q][0]]
        temp_dict["color"] = col
        output_kmc["last_step"] = temp_dict
        
        with open('output_kmc.yml', 'w') as f:
            yaml.dump(output_kmc, f) # aqui
        ...
        with open(output_dir + '/parameter_table.txt', 'w+') as test:
            ...
            try:
                test.write(
                    "Flux      in-flux    or_flux      sei/all    porosity   thickness   thick_in    thick_or\n")
                fmt = "%s%17s%12s%12s%12s%12s%12s%12s%12s"
                test.write(fmt % (
                    round(Flux, 5), round(in_flux, 5), round(or_flux, 5),
